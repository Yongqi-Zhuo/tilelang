...
def test_cute_right_inverse(shape, stride):
    L = cute.make_layout(shape, stride)
...
def test_cute_composition(ashape, astride, bshape, bstride):
    A = cute.make_layout(ashape, astride)
    B = cute.make_layout(bshape, bstride)
...
def test_cute_right_inverse_composition_smem_to_gmem():
    ...
    smem = cute.make_layout([8, 64], [64, 1])  # logical -> SMEM addr
    gmem = cute.make_layout([8, 64], [1024, 1])  # logical -> GMEM addr (sub-tile)
...
def test_cute_tma_box_validity_via_composite():
    ...
    gmem = cute.make_layout([8, 64], [256, 1])  # row-major sub-tile of [8,256]

    row_major = cute.make_layout([8, 64], [64, 1])
    ...
    transposed = cute.make_layout([8, 64], [1, 8])  # i fastest in SMEM
...
def test_derive_tma_tile_geometry():
    ...
    gmem2 = cute.make_layout([128, 64], [64, 1])
    sp_a = cute.make_layout([128, 64], [64, 1])
    ...
    gmem_b = cute.make_layout([32, 128], [128, 1])
    sp_b = cute.make_layout([32, 64, 2], [64, 1, 2048])
    ...
    gmem_t = cute.make_layout([8, 64], [64, 1])
    sp_t = cute.make_layout([64, 8], [1, 64])
...
def test_derive_tma_tile_splits_wide_inner_mode():
    gmem = cute.make_layout([64, 512], [512, 1])
    sp = cute.make_layout([64, 512], [512, 1])
...
def test_derive_tma_tile_rejects_gapped_layout():
    ...
    gmem = cute.make_layout([8, 64], [64, 1])
    sp = cute.make_layout([64, 8], [1, 80])
...
def test_derive_tma_tile_dynamic_gmem():
    n = tvm.tirx.Var("n", "int32")
    gmem = cute.make_layout([128, 64], [n, 1])  # row-major, dynamic N stride
    sp = cute.make_layout([128, 64], [64, 1])
...
def test_make_identity_layout_leaves_are_scaled_basis():
    ...
    for k, leaf in enumerate(leaves):
        assert isinstance(leaf, cute.ScaledBasis)
        assert isinstance(leaf.value, cute.IntConst) and leaf.value.value == 1
        assert list(leaf.basis) == [k]


def test_int_expr_stride_roundtrips():
    s = tvm.tirx.Var("s", "int32")
    L = cute.make_layout([8], [s])
    (leaf,) = L.stride_leaves
    assert isinstance(leaf, cute.IntExpr)
...
def test_composition_rhs_scaled_basis_dynamic_strides():
    ...
    A = cute.make_layout([16, 16], [s_m, s_n])
    B = cute.make_identity_layout([4, 4])
    R = cute.composition(A, B)
    assert list(R.shape) == [4, 4]
    leaves = R.stride_leaves
    assert len(leaves) == 2
    assert isinstance(leaves[0], cute.IntExpr) and tvm.ir.structural_equal(leaves[0].value, s_m)
    assert isinstance(leaves[1], cute.IntExpr) and tvm.ir.structural_equal(leaves[1].value, s_n)
